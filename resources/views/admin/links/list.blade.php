...
@section('content')
                        ...
                        <div class="row">
                            <div class="col-sm-12 m-b-xs">
                                <div class="btn-group hidden-xs" id="exampleTableEventsToolbar" role="group">
                                    <a class="btn btn-outline btn-default" href="{{url('admin/links/create')}}" title="添加">
                                        <i class="glyphicon glyphicon-plus" aria-hidden="true"></i>
                                    </a>
                                    <a class="btn btn-outline btn-default" href="{{url('admin/links')}}" title="刷新">
                                ...
                                @foreach($data as $k=>$v)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <input type="text" value="{{$v['link_sort']}}" onchange="changeSort(this.value, {{$v['link_id']}})" class="form-control" style="width: 36px;padding: 6px;text-align: center;margin: auto">
                                        </td>
                                        <td>{{$v['link_name']}}</td>
                                        <td class="">{{$v['link_title']}}</td>
                                        <td class="text-center">{{$v['link_url']}}</td>
                                        <td class="text-center" class="do">
                                            <a href="javascript:void(0)" onclick="del({{$v['link_id']}})" title="删除"> <i class="glyphicon glyphicon-trash" aria-hidden="true"></i> </a>
                                            | <a href="{{url('admin/links/'.$v['link_id'].'/edit')}}" title="修改" > <i class="glyphicon glyphicon-edit"></i> </a>
                                        </td>
                                    </tr>
    ...
    <script>
        ...
        function changeSort(val,link_id){

            $.post('{{url('admin/links/changeSort')}}',{'_token':'{{csrf_token()}}','link_sort':val,'link_id':link_id},function(msg){
                layer.msg(msg.msg)
           })
        }
